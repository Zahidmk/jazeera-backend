...
export const handleWebhook = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    switch (event) {
      case 'order.created':
        await enqueueOrderSync({ type: 'order_created', odooId: numericId, data: data || {} });
        break;

      case 'order.updated':
        await enqueueOrderSync({ type: 'order_updated', odooId: numericId, data: data || {} });
        break;

      case 'order.cancelled':
        await enqueueOrderSync({ type: 'order_cancelled', odooId: numericId });
        break;

      case 'product.updated':
        await enqueueProductSync({ type: 'single_update', odooId: numericId, data: data || {} });
        break;

      case 'customer.created':
      case 'customer.updated':
        await enqueueCustomerSync({ type: 'single_upsert', odooId: numericId, data: data || {} });
        break;
